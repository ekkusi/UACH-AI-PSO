...
class Particle{
  ...
  float Eval(PImage surf){ //recibe imagen que define función de fitness
    ...
    fit = 20 + pow(x, 2) - 10 * cos(2*PI*x) + pow(y, 2) - 10 * cos(2*PI*y);  //evalúa por el valor de la componente roja de la imagen
  ...
  void move(){
    ...
    y = y + vy;
    // rebota en murallas
    if (x*512/5.12+512 > width || x*512/5.12+512 < 0) vx = - vx;
    if (y*256/5.12+256 > height || y*256/5.12+256 < 0) vy = - vy;
  }
  ...
  void display(){
    ...
    fill(c);
    ellipse (x*512/5.12+512,y*256/5.12+256,d,d);
    // dibuja vector
    stroke(#ff0000);
...
void despliegaBest(){
  fill(#0000ff);
  ellipse(gbestx*512/5.12+512,gbesty*256/5.12+256,d,d);
